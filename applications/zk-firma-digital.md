...
Constancias Municipales: A project where our CEO advised on integrating Costa Rican digital identity with blockchain for issuing signed tax certifications, demonstrating expertise in similar initiatives (https://github.com/edenia/polygon-id-verifier).
Polygon ID Verifier: An infrastructure we have set up based on Polygon ID to authenticate users with digital credentials while maintaining privacy, utilizing zero-knowledge proofs to reveal only necessary information (https://github.com/edenia/polygon-id-verifier).
